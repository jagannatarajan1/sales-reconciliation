...
apt-get install -y -qq curl git ca-certificates gnupg nginx postgresql postgresql-contrib ufw \
  build-essential python3   # fallback for sharp if no prebuilt libvips binary matches
...
# ── Session photos ──
# Local disk on this VPS. The storage layer is driver-based, so moving to
# S3/R2/MinIO later is a config + driver change, not a rewrite.
PHOTO_STORAGE_DRIVER=local
PHOTO_UPLOAD_DIR=${UPLOAD_DIR}

# Must match the `internal` location in the nginx site. With this set, Node
# only authorises the request and nginx pushes the bytes.
PHOTO_ACCEL_REDIRECT=/_protected_uploads
