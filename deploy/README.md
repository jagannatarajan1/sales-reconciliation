...
four `GMAIL_*` values. Leave `DATABASE_URL`, `JWT_SECRET` and the `PHOTO_*` settings as generated.
...
```
<section>/<YYYY-MM-DD>/<SHIFT>/<uuid>.jpg
<section>/<YYYY-MM-DD>/<SHIFT>/<uuid>_thumb.jpg
```

One session's evidence sits in one directory, so it can be archived or pruned
with a single directory operation. Each photo is stored twice: a resized full
copy and a thumbnail the admin grid loads instead of full-resolution images.

**They are not publicly reachable.** Every request goes through
`GET /api/session-photos/:id/file` (staff) or
`GET /api/admin/session-photos/:id/file` (admin), which check the bearer token
and the section's permission first, then hand the file to nginx via
...
Every upload is decoded and re-encoded by `sharp` before it is stored. That
strips EXIF (including GPS) and destroys anything hidden behind valid image
magic bytes — a file that will not decode as an image is rejected outright.

### Configuration

| Variable | Default | Purpose |
|---|---|---|
| `PHOTO_STORAGE_DRIVER` | `local` | Storage backend. Only `local` ships today; the driver interface exists so S3/R2/MinIO can be added without touching routes or schema. |
| `PHOTO_UPLOAD_DIR` | `./uploads/session-photos` | Where the local driver writes. **Must be absolute in production.** |
| `PHOTO_ACCEL_REDIRECT` | *(empty)* | nginx `internal` location. Empty means Node streams the bytes itself — slower, but useful for diagnosis. |
| `MAX_PHOTO_SIZE_MB` | `10` | Per-file ceiling. Keep `client_max_body_size` above this × `PHOTO_MAX_FILES_PER_REQUEST`. |
| `PHOTO_MAX_FILES_PER_REQUEST` | `10` | Files per upload. |
| `PHOTO_ALLOWED_TYPES` | jpeg, png, webp, heic | Checked against sniffed bytes, never the filename. |
| `PHOTO_MAX_EDGE_PX` | `1600` | Long edge of the stored copy. |
| `PHOTO_THUMB_EDGE_PX` | `320` | Long edge of the thumbnail. |

HEIC decoding depends on how libvips was built. If HEIC uploads are rejected on
your VPS, that is why — the API returns a clear message telling the user to
send a JPEG or PNG.

### Admin access

The admin browser lives at `/admin/session-photos` and is gated on a
`sessionPhotos` permission module.

⚠️ **Existing admin accounts do not have this permission.** After deploying,
grant it in `/admin/users` or the dashboard card stays hidden.

### Disk usage

Rough sizing: 10 photos/day at ~400KB each is about 1.5GB/year, plus ~5% for
thumbnails. To check:
...
than what is being sent. It ships at `160m`, which covers `MAX_PHOTO_SIZE_MB` × `PHOTO_MAX_FILES_PER_REQUEST` plus multipart overhead.

**Photos upload but do not display** — check `PHOTO_ACCEL_REDIRECT` is
`/_protected_uploads` and that the nginx `alias` path matches `PHOTO_UPLOAD_DIR`
exactly, trailing slash included. As a fallback, clearing
`PHOTO_ACCEL_REDIRECT` makes Node stream the files itself, which is slower but
