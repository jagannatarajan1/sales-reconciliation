...
import {
  FiArrowLeft, FiSearch, FiCheckCircle, FiUser, FiKey,
  FiClock, FiAlertTriangle, FiCreditCard, FiDollarSign, FiTrendingDown, FiPackage,
  FiAward, FiGrid, FiInbox, FiFileText, FiPrinter, FiDownload, FiBarChart2,
} from 'react-icons/fi';
import { useAuth } from '../../context/AuthContext';
import { useToast } from '../../components/ui/Toast';
import './SalesReconciliation.css';
...
/* Reconciliation status, in plain terms — "Reconciled" once admin has signed
   off, "Committed" while it's only been committed, never anything vaguer. */
...
function ReportCard({ row, isSelected, onClick, compact = false }) {
  ...
  return (
    ...
    >
      ...
      <span className={`rpt-status-pill rpt-status-pill--${status.toLowerCase()}`}>{status}</span>
    ...
    <>
      ...
      <div className="rpt-detail-head">
        ...
        <div className="rpt-detail-badges rpt-no-print">
          <span className={`rpt-status-pill rpt-status-pill--${reconciliationStatus(detail).toLowerCase()}`}>
...
/* ── Main component ─────────────────────────────────────────────────── */
export const SalesReconciliation = () => {
  const { user } = useAuth();
