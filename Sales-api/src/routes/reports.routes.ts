...
// Backs the Admin Reconciliation "Download Bill" tab — browse committed
// reconciliation history, view the full field-by-field Z-Report comparison,
// and export a date range as PDF or Excel. Mounted at /api/admin/reports,
// gated on "commitHistory" — the same permission every other endpoint on
// that admin page uses.
function requireAdmin(req: import("express").Request, res: import("express").Response): boolean {
  return requirePermission(req, res, "commitHistory");
}

// A record only belongs here once staff have committed it and/or admin has
// signed off on it. A row that exists purely because admin opened the
// "Uncommitted Data" tab (which creates no ReconciliationRecord at all)
// never reaches this table, but this predicate also guards against any
// record that technically exists yet was never actually committed.
...
// per-date ZIP that /download-pdf produces — the "combined" export option
// on the Download Bill tab.
...
reportsRouter.get("/:date", async (req, res) => {
  ...
  try {
    ...
    if (parsed != null) {
      ...
      zReportTotal = parsed;
      totalVariance = Math.round((Number(record.summaryTotal) - parsed) * 100) / 100;
    }
  } catch {
  ...
  // The Z-Report email carries a single department total, not a per-field
  // breakdown, so there is no real "Z-Report Value" for an individual row —
  // that comparison only exists at the Totals level below. Each row's
  // zReportValue column stays blank; the frontend renders it as "—".
  ...
  res.json({
    ...
    zReportAvailable,
    zReportTotal: zReportAvailable ? zReportTotal : null,
    totalVariance: zReportAvailable ? totalVariance : null,
